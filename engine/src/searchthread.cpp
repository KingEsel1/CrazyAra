...
void node_assign_novelty_score(Node* node, const float* valueOutputs, size_t batchIdx, const SearchSettings* searchSettings, const float* inputPlanes, float* factPlanes, int numberInputTotal, int timeStep, double featureProbabilities) //MR-pseudo 
{
    ...
    if (searchSettings->usePocketForNovelty) {
        ...
        for (int i = offsetForPocketPieces; i < offsetForPocketPieces + inputPlanesSizePocket; i += 64) //MR shift i by 64 to get to next Plane! Because of the normalization of the pocket planes it is only necessary to look at one entry per plane
        {
